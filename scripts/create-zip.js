...
function getPluginVersion() {
	const pluginFile = fs.readFileSync("simple-menu.php", "utf8");
...
function runBuildSteps() {
	...
	try {
		...
		execSync(
            "wp i18n make-pot . languages/genesis-simple-menus.pot --exclude=config,node_modules,scripts,vendor --headers='{ \"Report-Msgid-Bugs-To\": \"StudioPress <translations@studiopress.com>\" }' --exclude=bin/ --quiet",
			{ stdio: "inherit" }
		);
	...
	function addDirectoryToZip(directory) {
		...
		for (const file of files) {
			...
			if (stats.isDirectory()) {
				...
				zip.addFile(
					path.join("genesis-simple-menus", relativePath),
